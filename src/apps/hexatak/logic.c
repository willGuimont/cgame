...
bool Cell_Pop(Cell *cell, Stone *out) {
    ...
    if (out) *out = cell->stones[--cell->count];
    else cell->count--;
    return true;
}
...
void Board_Init(Board *board, const int radius) {
    ...
    for (int q = -radius; q <= radius; q++) {
        for (int r = -radius; r <= radius; r++) {
            Hex h = Hex_Create(q, r);

            if (!Hex_IsInBound(h, radius)) continue;
...
bool Board_MoveStackOne(Board *board, const int from_index, const int dir) {
    if (from_index < 0 || from_index >= board->count) return false;
    if (dir < 0 || dir >= 6) return false;

    Cell *from = &board->cells[from_index];
    if (from->count == 0) return false;

    Hex delta = Hex_Direction(dir);
    Hex to_hex = Hex_Add(from->hex, delta);

    int to_index = Board_FindCellIndex(board, to_hex);
    if (to_index < 0) return false;

    Cell *to = &board->cells[to_index];
    if (to->blocked) return false;

    if (to->count + from->count > MAX_STACK) return false;
...
bool Board_SpreadStack(Board *board, const int from_index, const int dir, const int distance) {
    ...
    if (distance > from->count) return false;
    ...
    for (int step = 0; step < distance; step++) {
        ...
        int idx = Board_FindCellIndex(board, cursor);
...
bool Hex_OnSide(const Hex h, const int radius, const BoardSide side) {
    ...
    switch (side) {
        case SIDE_Q_NEG: return h.q == -radius;
        case SIDE_Q_POS: return h.q ==  radius;
        case SIDE_R_NEG: return h.r == -radius;
        case SIDE_R_POS: return h.r ==  radius;
        case SIDE_S_NEG: return s   == -radius;
        case SIDE_S_POS: return s   ==  radius;
    }
...
bool Cell_IsRoad(const Cell *cell) {
    return !cell->blocked && cell->count > 0;
}
...
bool Board_HasConnection(const Board *board, const BoardSide a, const BoardSide b) {
    ...
    for (int i = 0; i < board->count; i++) {
        ...
        if (!Cell_IsRoad(cell)) continue;
        if (!Hex_OnSide(cell->hex, board->radius, a)) continue;
    ...
    while (head < tail) {
        ...
        for (int dir = 0; dir < 6; dir++) {
            ...
            if (!Cell_IsRoad(&board->cells[ni])) continue;
...
const LevelDesc LEVELS[LEVEL_COUNT] = {
    ...
    // Level 2: Bridge Building
    {
        ...
        .initial_stacks = {
            { .hex = { -2, 0 }, .count = 2, .stone_values = { 2, 4 } },
