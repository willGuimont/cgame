...
i32 Utils_MeasureTextWrappedHeight(const char *text, const i32 max_width, const i32 font_size, const i32 line_spacing) {
    if (!text)
        return 0;
...
void Utils_DrawTextWrappedCentered(const char *text, const i32 center_x, const i32 start_y, const i32 max_width,
    ...
    while (*ptr != '\0') {
        ...
        if (len > 0) {
            ...
            const i32 tw = MeasureText(draw_buf, font_size);
...
void Render_DrawStoneStack(const Cell *cell, const Vector2 center, const float size, const i32 alpha, const bool active,
                           const float pulse) {
    if (cell->count == 0)
        return;
    ...
    for (i32 i = 0; i < cell->count; i++) {
        ...
        constexpr i32 FONT_SZ = 14;
        const i32 tw = MeasureText(val_str, FONT_SZ);
...
void Render_DrawButton(const Rectangle rect, const char *text, const Color bg, const Color text_col,
                       const bool hovered) {
    DrawRectangleRec(rect, bg);
    DrawRectangleLinesEx(rect, 2.0f, hovered ? (Color) {249, 226, 175, 255} : (Color) {88, 91, 112, 255});

    constexpr i32 FONT_SZ = 16;
    const i32 tw = MeasureText(text, FONT_SZ);
...
void Render_DrawBoard(GameState *gs, const Vector2 origin, const float size) {
    ...
    for (i32 i = 0; i < board->count; i++) {
        ...
        if (cell->required_value > 0) {
            ...
            constexpr i32 REQ_FONT_SZ = 11;
            i32 req_tw = MeasureText(req_str, REQ_FONT_SZ);
            DrawText(req_str, (i32) (badge_center.x - (float) req_tw / 2.0f),
            ...
            if (disp_cell->count > 0) {
                ...
                if (top_val != cell->required_value) {
                    ...
                    DrawText("!", (i32) (warn_center.x - 2.0f), (i32) (warn_center.y - 5.0f), 10,
                             (Color) {17, 17, 27, 255});
                }
            }
        }
...
void Render_DrawUI(const GameState *gs) {
    ...
    Render_DrawButton(btn_menu, gs->testing_editor_level ? "EDITOR (Esc)" : "MENU (Esc)", (Color) {49, 50, 68, 255},
    ...
    const i32 instr_tw = MeasureText(instr, 16);
...
void Render_DrawEditorUI(const GameState *gs) {
    ...
    // Editor Title
    DrawText("LEVEL EDITOR", 20, 25, 22, (Color) {250, 179, 135, 255}); // Peach
    DrawText("Build your custom level", 20, 55, 12, (Color) {166, 173, 200, 255});

    // Active Tool selection
    DrawText("ACTIVE TOOL", 20, 95, 14, (Color) {166, 173, 200, 255});
    ...
    Render_DrawButton(rect_tool_stones, "STONES TOOL", (gs->editor_active_tool == 0) ? active_bg : inactive_bg,
                      (gs->editor_active_tool == 0) ? active_fg : inactive_fg,
                      CheckCollisionPointRec(mouse, rect_tool_stones));

    Render_DrawButton(rect_tool_blocked, "BLOCKED TOOL", (gs->editor_active_tool == 1) ? active_bg : inactive_bg,
                      (gs->editor_active_tool == 1) ? active_fg : inactive_fg,
                      CheckCollisionPointRec(mouse, rect_tool_blocked));

    Render_DrawButton(rect_tool_required, "REQUIRED TOOL", (gs->editor_active_tool == 2) ? active_bg : inactive_bg,
                      (gs->editor_active_tool == 2) ? active_fg : inactive_fg,
                      CheckCollisionPointRec(mouse, rect_tool_required));

    Render_DrawButton(rect_tool_goals, "GOAL EDGES TOOL", (gs->editor_active_tool == 3) ? active_bg : inactive_bg,
                      (gs->editor_active_tool == 3) ? active_fg : inactive_fg,
                      CheckCollisionPointRec(mouse, rect_tool_goals));

    // Properties
    DrawText("PROPERTIES", 20, 255, 14, (Color) {166, 173, 200, 255});

    const Rectangle rect_radius = {20.0f, 275.0f, 160.0f, 26.0f};
    char rad_str[32];
    snprintf(rad_str, sizeof(rad_str), "RADIUS: %d", gs->editor_board.radius);
    Render_DrawButton(rect_radius, rad_str, inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, rect_radius));
    ...
    Render_DrawButton(rect_side_a, side_a_str, inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, rect_side_a));
    ...
    Render_DrawButton(rect_side_b, side_b_str, inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, rect_side_b));

    // Move Limit dec/inc
    DrawText("MOVE LIMIT", 20, 375, 14, (Color) {166, 173, 200, 255});
    const Rectangle rect_moves_dec = {20.0f, 395.0f, 40.0f, 26.0f};
    const Rectangle rect_moves_inc = {140.0f, 395.0f, 40.0f, 26.0f};
    Render_DrawButton(rect_moves_dec, "-", inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, rect_moves_dec));
    Render_DrawButton(rect_moves_inc, "+", inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, rect_moves_inc));

    char limit_str[16];
    snprintf(limit_str, sizeof(limit_str), "%d", gs->editor_move_limit);
    DrawText(limit_str, 95 - (MeasureText(limit_str, 16) / 2), 400, 16, (Color) {205, 214, 244, 255});

    // Palette display depending on tool
    if (gs->editor_active_tool == 0) {
        DrawText("PLACEMENT STACK", 20, 435, 14, (Color) {166, 173, 200, 255});
        ...
        for (i32 s = 0; s < gs->editor_placement_stack_count; s++) {
            ...
            DrawText(val_lbl,
                     (i32) (stone_rect.x + (stone_rect.width / 2.0f) - (float) MeasureText(val_lbl, 10) / 2.0f),
                     (i32) (stone_rect.y + 5.0f), 10, (Color) {17, 17, 27, 255});
        }

        // "CLEAR" button
        Rectangle rect_clear = {140.0f, 433.0f, 40.0f, 18.0f};
        Render_DrawButton(rect_clear, "CLR", inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, rect_clear));

        // "ADD" values (1, 2, 3, 4, 8, 16, 32, 64)
        DrawText("ADD TO STACK", 20, 485, 13, (Color) {166, 173, 200, 255});
        ...
        for (i32 v = 0; v < 8; v++) {
            ...
            DrawText(val_lbl, (i32) (val_rect.x + (val_rect.width / 2.0f) - (float) MeasureText(val_lbl, 9) / 2.0f),
                     (i32) (val_rect.y + 5.0f), 9, (Color) {17, 17, 27, 255});
        }

        // Presets
        DrawText("PRESETS", 20, 530, 13, (Color) {166, 173, 200, 255});
        Rectangle btn_preset_asc = {20.0f, 545.0f, 75.0f, 18.0f};
        Rectangle btn_preset_desc = {105.0f, 545.0f, 75.0f, 18.0f};
        Render_DrawButton(btn_preset_asc, "1,2,3,4", inactive_bg, inactive_fg,
                          CheckCollisionPointRec(mouse, btn_preset_asc));
        Render_DrawButton(btn_preset_desc, "4,3,2,1", inactive_bg, inactive_fg,
                          CheckCollisionPointRec(mouse, btn_preset_desc));
    } else if (gs->editor_active_tool == 2) {
        DrawText("REQUIRED VALUE", 20, 440, 14, (Color) {166, 173, 200, 255});
        ...
        for (i32 v = 0; v < 6; v++) {
            ...
            char val_lbl[8];
            snprintf(val_lbl, sizeof(val_lbl), "%d", values[v]);
            DrawText(val_lbl, (i32) (val_rect.x + (val_rect.width / 2.0f) - (float) MeasureText(val_lbl, 10) / 2.0f),
                     (i32) (val_rect.y + 8.0f), 10, (Color) {17, 17, 27, 255});
        }
    }
    ...
    Render_DrawButton(btn_test, "TEST LEVEL", (Color) {166, 227, 161, 255}, (Color) {30, 30, 46, 255},
                      CheckCollisionPointRec(mouse, btn_test));
    Render_DrawButton(btn_export, "EXPORT", (Color) {203, 166, 247, 255}, (Color) {30, 30, 46, 255},
                      CheckCollisionPointRec(mouse, btn_export));
    Render_DrawButton(btn_menu, "MAIN MENU", inactive_bg, inactive_fg, CheckCollisionPointRec(mouse, btn_menu));

    // Display Help Text
    if (gs->editor_active_tool == 3) {
        DrawText("L-Click: Set Goal Side A", 230, 20, 14, (Color) {166, 173, 200, 255});
        DrawText("R-Click: Set Goal Side B", 230, 40, 14, (Color) {166, 173, 200, 255});
    } else {
        DrawText("L-Click: Place Stone/Block/Req", 230, 20, 14, (Color) {166, 173, 200, 255});
        DrawText("R-Click: Clear Stack/Block/Req", 230, 40, 14, (Color) {166, 173, 200, 255});
    }
}
